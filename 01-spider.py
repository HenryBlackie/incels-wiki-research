...
class IncelswikiSpider(scrapy.Spider):
    ...
    custom_settings = {
        'FEEDS': {
            f'feeds/nodes.csv': {'format': 'csv', 'fields': ['id', 'label']},
            f'feeds/edges.csv': {'format': 'csv', 'fields': ['source', 'target']}
        },
    ...
    def parse(self, response):
        ...
        body_links = response.css('#mw-content-text').css('a[href^="/w/"]:not(.image):not(.reference):not(.external):not(.new)').xpath('@href').getall()
    ...
    def local_archive(self, response):
        ...
        filename = re.sub(r'[<>:"/\\|?*\x00-\x1F]', '_', filename).strip()  # Replace invalid characters
